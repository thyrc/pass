...
class Vault {
    public function unwrap($token = '') {
        $ch = curl_init();
        curl_setopt($ch, CURLOPT_URL, getenv('VAULT_ADDR').'/v1/sys/wrapping/unwrap');
    ...
    public function wrap($data) {
        $ch = curl_init();
        curl_setopt($ch, CURLOPT_URL, getenv('VAULT_ADDR').'/v1/sys/wrapping/wrap');
        curl_setopt($ch, CURLOPT_POST, 1);
        curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);

        $headers = [
            "X-Vault-Token: ".getenv('VAULT_TOKEN'),
            "X-Vault-Wrap-TTL: ".getenv('VAULT_WRAP_TTL')
        ];
